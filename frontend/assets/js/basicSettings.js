...
button.addEventListener("click", function(event){
	...
	saveBtn.addEventListener("click", function(event){
		...
		if(title.value.trim() !== ""){
			...
			if(httpRequest){
				httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/updateTitle.php', true);
			...
			document.querySelector("#titleError").innerHTML = "Requied fields must not be blank";
		}
	});
});
...
descBtn.addEventListener("click", function(event){
	...
	descSaveBtn.addEventListener("click", function(event){
		...
		if(descText.value.trim().length < '500'){
			...
			if(httpRequest){
				httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/updateDescription.php', true);
				httpRequest.onreadystatechange = function(){
					if(this.readyState === 4 && this.status === 200){
						if(/OwnnerError$/i.exec(this.responseText)){
							alert("You cannot preform this action!");
