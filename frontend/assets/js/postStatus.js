	...
	draftBtn.addEventListener("click", function(e){
		...
		if(postIDs.length > 0){
			if(confirm("Are you sure, you want to draft this?")){
				...
				if(httpRequest){
					httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/draftPosts.php', true);
	...
	publishBtn.addEventListener("click", function(event){
		...
		if(postIDs.length > 0){
			if(confirm("Are you sure, you want to publish this?")){
				...
				if(httpRequest){
					httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/publishPosts.php', true);
					httpRequest.onreadystatechange = function(){
						if(this.readyState === 4 && this.status === 200){
							if(this.responseText.length != 0){
								alert(this.responseText);
