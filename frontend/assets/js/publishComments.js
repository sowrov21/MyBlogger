...
button.addEventListener("click", function(event){
	...
	if(postIDs.length > 0){
		...
		if(httpRequest){
			httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/publishComments.php', true);
...
publishLink.forEach(function(el){
	el.addEventListener("click", function(event){
		...
		if(httpRequest){
			httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/publishCommentByLink.php', true);
			httpRequest.onreadystatechange = function(){
				if(this.readyState === 4 && this.status === 200){
					if(this.responseText.length != 0){
						alert(this.responseText);
