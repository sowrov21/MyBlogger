	...
	formSaveBtn.addEventListener("click", function(event){
		...
		if(email.value && name.value && pass.value && passRe.value !==''){
			if(pass.value === passRe.value){
				...
				if(httpRequest){
					httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/createAuthor.php', true);
				...
				document.querySelector("#passReError").innerHTML = "Passsword is does not match!";
			}
		}
	});
