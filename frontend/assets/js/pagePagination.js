...
button.addEventListener("click", function(event){
	...
	pages.forEach(function(el){
		el.addEventListener("click", function(event){
			...
			if(httpRequest){
				httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/showNextPages.php', true);
...
previousBtn.addEventListener("click", function(event){
	...
	if(currentNum > '1'){
		...
		var formData  = new FormData();
			...
			if(httpRequest){
				httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/showPreviousPages.php', true);
				httpRequest.onreadystatechange = function(){
					if(this.readyState === 4 && this.status === 200){
						document.querySelector("#posts").innerHTML = this.responseText;
						currentPage.innerHTML = currentNum;
...
postLimit.addEventListener("change", function(e){
	...
	if(httpRequest){
		httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/jumpToPage.php', true);
		httpRequest.onreadystatechange = function(){
			if(this.readyState === 4 && this.status === 200){
				document.querySelector("#posts").innerHTML = this.responseText;
...
function getPagesNumbers(jumpTo){
	...
	if(httpRequest){
		httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/getPagesNumbers.php', true);
		httpRequest.onreadystatechange = function(){
			if(this.readyState === 4 && this.status === 200){
