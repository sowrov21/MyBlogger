...
deleteLink.forEach(function(el){
	el.addEventListener("click", function(e){
		...
		if(confirm("Are you sure, you want to delete this?")){
				...
				if(httpRequest){
					httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/removeCommentByLink.php', true);
...
deleteBtn.addEventListener("click", function(e){
		...
		if(postIDs.length > 0){
			if(confirm("Are you sure, you want to delete this?")){
				...
				if(httpRequest){
					httpRequest.open('POST', 'http://localhost/MyBlogger/backend/ajax/removeComments.php', true);
					httpRequest.onreadystatechange = function(){
						if(this.readyState === 4 && this.status === 200){
							if(this.responseText.length != 0){
								alert(this.responseText);
